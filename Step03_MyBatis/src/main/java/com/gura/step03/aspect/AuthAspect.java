...
@Component //	component scan을 통해서 bean 이 될 수 있도록	
public class AuthAspect {
	 ...
	 *  따라서 spring이 관리하는 객체중에 위의 모양을 가지고 있는 메소드는 
	 *  아래의 aspect 가 적용이 된다.
	 */
		//aspectJ expression은 어떤 모양의 메소드에 적용을 시킬 지 결정한다.
		//auth로 시작하는 모든(*) 메소드에 적용하는 모습
	   @Around("execution(org.springframework.web.servlet.ModelAndView auth*(..))")
	   
	   public Object loginCheck(ProceedingJoinPoint joinPoint) throws Throwable {
		   //aspectj가 적용된 메소드에 전달된 값을 배열로 얻어내기.
	      Object[] args=joinPoint.getArgs();
	      //전달된 인자가 없어도 null 은 아님. 방의 개수가 zero이니까 다만 반복문은 돌지 않는다.
...
//	      }
	      ...
	      for(Object tmp:args) {
	    	  //만일 tmp가 HttpServletRequest type 이라면?
